  ...
  async (user, thunkAPI) => {
    try {
      const resp = await customFetch.patch('/auth/updateUser', user, {
        headers: {
          authorization: `Bearer ${thunkAPI.getState().user.user.token}`, // user(userSlice).user(property of the slice).token(property of the user)
        },
      });
      return resp.data; //return a different token (Json Web Token JWT)
    } catch (error) {
      console.log(error.response);
      return thunkAPI.rejectWithValue(error.response.data.msg);
    }
...
export const updateUser = createAsyncThunk(
  ...
  async (user, thunkAPI) => {
    try {
      ...
      // console.log(error.response); - invalid authorization, no token, log out the user
